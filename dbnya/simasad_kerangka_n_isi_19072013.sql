...
INSERT INTO `appsetting` (`id`, `created_at`, `updated_at`, `biaya_id`, `mysqldumppath`, `cetaknota`, `printeraddr`, `lunas`, `linekertas`, `spaceprinter`, `charcount`, `sn_key`) VALUES
	(1, '2013-05-31 10:03:45', '2013-07-18 14:39:21', 1, 'C:\\xampp\\mysql\\bin\\', 'N', '//192.168.0.2/epson_lx_800', 'N', 30, 14, 56, 'TkQtZ3QtTlUtSXQtTXota3QtTVUtTXQtUkQtVXQtT0UtRT0=');
...
-- Dumping data for table simasad.detiltransmasuk: ~29 rows (approximately)
...
-- Dumping data for table simasad.ketentuanbiaya: ~7 rows (approximately)
...
-- Dumping data for table simasad.potongan: ~5 rows (approximately)
...
INSERT INTO `potongan` (`id`, `created_at`, `updated_at`, `siswa_id`, `tahunajaran_id`, `bulan_id`, `jenisbiaya_id`, `disc`, `nilai`, `ket`, `jenis`) VALUES
	(1, '2013-07-18 10:58:05', '2013-07-18 10:58:05', 183, 2, 7, 1, 50, 70000, 'Peringkat Kelas 1', 'BS'),
	(2, '2013-07-18 10:58:05', '2013-07-18 10:58:05', 183, 2, 8, 1, 50, 70000, 'Peringkat Kelas 1', 'BS'),
	(3, '2013-07-18 14:24:27', '2013-07-18 14:24:27', 226, 2, 7, 1, 50, 70000, 'Peringkat Kelas ke 1', 'BS'),
	(4, '2013-07-18 14:24:27', '2013-07-18 14:24:27', 226, 2, 8, 1, 50, 70000, 'Peringkat Kelas ke 1', 'BS'),
	(5, '2013-07-18 14:24:27', '2013-07-18 14:24:27', 226, 2, 9, 1, 50, 70000, 'Peringkat Kelas ke 1', 'BS');
...
-- Dumping data for table simasad.rombelsiswa: ~484 rows (approximately)
...
-- Dumping data for table simasad.siswa: ~335 rows (approximately)
...
-- Dumping data for table simasad.transmasuk: ~29 rows (approximately)
...
INSERT INTO `transmasuk` (`id`, `created_at`, `updated_at`, `tanggal`, `tahunajaran_id`, `siswa_id`, `arus`, `total`) VALUES
	(9, '2013-07-18 15:46:27', '2013-07-18 15:46:27', '2013-07-18', 2, 198, 'M', 140000),
	(10, '2013-07-18 15:46:36', '2013-07-18 15:46:36', '2013-07-18', 2, 210, 'M', 140000),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` VIEW `simasad`.`view_spp` AS SELECT s.id,s.nisn,s.nama,s.jenjang_spp,s.mutasi,rs.tahunajaran_id,ta.nama as tahunajaran,r.nama as rombel,r.jenjang,
(
SELECT jumlah
FROM ketentuanbiaya kb
WHERE kb.jenjang = s.jenjang_spp AND kb.tahunajaran_id = ta.id AND kb.jenisbiaya_id=1) AS jumlah, 1 as jenisbiaya_id,'ITB' as tipe
FROM siswa s
INNER JOIN rombelsiswa rs ON s.id = rs.siswa_id
INNER JOIN tahunajaran ta ON rs.tahunajaran_id = ta.id
INNER JOIN rombel r ON rs.rombel_id = r.id
ORDER BY s.id,s.nisn,ta.id ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` VIEW `simasad`.`vtransmasuk` AS select `tm`.`id` AS `id`,`tm`.`created_at` AS `created_at`,`tm`.`updated_at` AS `updated_at`,`tm`.`tanggal` AS `tanggal`
,`tm`.`arus` AS `arus`,`tm`.`tahunajaran_id` AS `tahunajaran_id`,`ta`.`nama` AS `tahunajaran`,
`ta`.`posisi` AS `posisi_tahunajaran`,`tm`.`siswa_id` AS `siswa_id`,`sw`.`nisn` AS `nisn`,`sw`.`nama` AS `siswa`,
`dtm`.`id` AS `detil_id`,`dtm`.`jenisbiaya_id` AS `jenisbiaya_id`,`jb`.`nama` AS `jenisbiaya`,`jb`.`tipe` AS `tipe`,
`dtm`.`bulan_id` AS `bulan_id`,`bl`.`nama` AS `bulan`,`bl`.`posisi` AS `posisi`,`dtm`.`ket` AS `ket`,
dtm.harus_bayar, dtm.potongan,`dtm`.`jumlah` AS `jumlah` from (((((`transmasuk` `tm` join `detiltransmasuk` `dtm` 
on((`dtm`.`transmasuk_id` = `tm`.`id`))) join `jenisbiaya` `jb` on((`dtm`.`jenisbiaya_id` = `jb`.`id`))) 
left join `siswa` `sw` on((`tm`.`siswa_id` = `sw`.`id`))) left join `bulan` `bl` on((`dtm`.`bulan_id` = `bl`.`id`)))
 join `tahunajaran` `ta` on((`tm`.`tahunajaran_id` = `ta`.`id`))) ;
