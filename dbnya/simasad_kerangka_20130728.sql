...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` VIEW `simasad`.`view_transmasuk` AS select tm.id as transmasuk_id, dtm.id as detiltransmasuk_id, tm.created_at, tm.updated_at, tm.tanggal, tm.tahunajaran_id, ta.nama as tahunajaran, 
tm.siswa_id, s.nama as siswa, tm.arus, tm.total, dtm.jenisbiaya_id, jb.nama as jenisbiaya, 
dtm.bulan_id,b.nama as bulan,dtm.harus_bayar,dtm.jumlah,dtm.potongan,dtm.ket
from detiltransmasuk dtm  inner join  transmasuk tm on dtm.transmasuk_id = tm.id
inner join tahunajaran ta on tm.tahunajaran_id = ta.id left join siswa s on tm.siswa_id = s.id 
inner join jenisbiaya jb on dtm.jenisbiaya_id = jb.id left join bulan b on dtm.bulan_id = b.id ;
