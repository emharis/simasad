...
class Rekap_Tahunan_Controller extends Base_Controller {
    ...
    public function get_printtopdf($tahunajaranid){
        ...
        $pendapatanLalu = DB::query("select sum(jumlah) as jumlah
from vtransmasuk where arus = 'M' and tahunajaran_id in (select id
from tahunajaran where posisi < (select posisi from tahunajaran where id = " . $tahunajaranid . "))");
        
        $pengeluaranLalu = DB::query("select sum(jumlah) as jumlah
from vtransmasuk where arus = 'K' and tahunajaran_id in (select id
from tahunajaran where posisi < (select posisi from tahunajaran where id = " . $tahunajaranid . "))");
        
        $pendptn = DB::query("select sum(jumlah) as jumlah from vtransmasuk where arus = 'M' and tahunajaran_id = " . $tahunajaranid);
        $penglrn = DB::query("select sum(jumlah) as jumlah from vtransmasuk where arus = 'K' and tahunajaran_id = " . $tahunajaranid);
        
        
        $saldoLalu = $pendapatanLalu[0]->jumlah - $pengeluaranLalu[0]->jumlah;
...
//        $pdf->Cell($colk+$colm,7,  number_format($totmlalu-$totklalu, 0, ',', '.'),1,1,'R');
        $pdf->Cell($colk+$colm,7,  number_format($saldoLalu, 0, ',', '.'),1,1,'R');
...
//        $pdf->Cell($colk+$colm,7,  number_format(($totmlalu-$totklalu)+($totm-$totk), 0, ',', '.'),1,1,'R');
        $pdf->Cell($colk+$colm,7,  number_format(($saldoLalu)+($pendptn[0]->jumlah-$penglrn[0]->jumlah), 0, ',', '.'),1,1,'R');
