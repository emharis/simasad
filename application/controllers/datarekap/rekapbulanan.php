...
/**
 * Description of rekapbulanan
 *
 * @author Klik
 */
class Datarekap_Rekapbulanan_Controller extends Base_Controller {
    
    public $report_title = 'REKAPITULASI TRANSAKSI BULANAN';
    ...
    public function get_index(){
        ...
        $this->layout->nest('content', 'datarekap.bulanan.index',array(
    ...
    public function get_datarekapdetil($tahunId,$bulan){
        //------MENGHILANGKAN LAYOUT YANG SEBELUMNYA TELAH DISETTING-------------------------
        $this->layout = null;
        //------------------------------------------------------------------------------------
        
        //----GET DATA REKAPITULASI---------
        $datarekap = DB::query('select * from view_transmasuk where tahunajaran_id=' . $tahunId . ' and DATE_FORMAT(tanggal,"%m-%Y") = "' . $bulan . '"'  );
        //------------------------------------------------------------------------------------
        
        //----CONVERT DATA BULAN KE DALAM BENTUK STRING : SEPTEMBER 2013----------------------
        $bulan = date('F Y',strtotime('01-'.$bulan));
        //------------------------------------------------------------------------------------
        
        //-----RETURN VIEW DATA REKAP DETIL---------------------------------------------------
        return View::make('datarekap.bulanan.datarekapdetil')
                ->with('datarekap',$datarekap)
                ->with('bln',strval($bulan));
        //------------------------------------------------------------------------------------
    }
    
    public function get_datarekapgroup($tahunId,$bulan){
        $this->layout = null;
        $datarekap = DB::query("select vt.tahunajaran_id,vt.tahunajaran,vt.jenisbiaya_id,vt.jenisbiaya,vt.arus,sum(vt.jumlah) as jumlah 
                            from view_transmasuk vt where tahunajaran_id = " . $tahunId . " and DATE_FORMAT(tanggal,'%m-%Y') = '" . $bulan . "' group by (vt.jenisbiaya_id)");
        
        $bulan = date('F Y',strtotime('01-'.$bulan));
        return View::make('datarekap.bulanan.datarekapgroup')
                ->with('datarekap',$datarekap)
                ->with('bulan',$bulan);
    }
    
    public function get_printgroup($tahunId,$bulan){
        $this->layout = null;
//        $datarekap = DB::query("select vt.tahunajaran_id,vt.tahunajaran,vt.jenisbiaya_id,vt.jenisbiaya,vt.arus,sum(vt.jumlah) as jumlah 
...
//                            group by (vt.jenisbiaya_id)");
        ...
        // content keternagan
        $pdf->SetFont('courier', '', 8);
        $periodestr = 'Bulan : ' . date('F Y',strtotime('01-'.$bulan));
                    ...
                    //----CETAK PAGE HEADER UNTUK HALAMAN SETELAH HALAMAN 1----------
                        if($rownum == 2){
                            if($pageHeader == false){
                                ...
                                $pdf->setHeaderData('', '', 'Rekapitulasi Transaksi Bulanan','Bulan : ' . date('F Y',strtotime('01-'.$bulan)));
                                $pageHeader = true;
                            }
                        }
                    //-----------------------------------------------------
        ...
        $pdf->Output('RekapitulasiTransaksiBulanan_' . date('d_m_Y') . '.pdf','D');
    }
    
    public function get_printdetil($tahunId,$bulan){
                ...
                //-----CETAK ROW/BARIS GRAND TOTAL
                    $pdf->SetFont('courier', 'B', 8);
                    $pdf->Cell($tb_col_no + $tb_col_tgl + $tb_col_by + $tb_col_nis + $tb_col_sw + $tb_col_ket, $tb_rw_h, 'TOTAL', 1,0,'C');
                    $pdf->Cell($tb_col_pd, $tb_rw_h, number_format($pendapatan,0,',','.') , 1,0,'R');
                    $pdf->Cell($tb_col_pg, $tb_rw_h, number_format($pengeluaran,0,',','.') , 1,1,'R');
                //-----------------------------------------------------
